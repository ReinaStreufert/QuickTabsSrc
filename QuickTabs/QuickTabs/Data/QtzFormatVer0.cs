...
{
    // old trash code
    public class QtzFormatVer0 : QtzVersion
    {
        public const ushort FormatVersion = 0;
        ...
        public override Song Open(FileStream fs, out bool failed)
        {
            ...
            song.FocusedTab.Tuning = new Tuning(tuningNotes);
            ...
            song.FocusedTab.SetLength(BitConverter.ToInt32(stepCountBytes, 0), MusicalTimespan.Zero);
            ...
            for (int i = 0; i < sectionHeadCount; i++)
            {
                ...
                song.FocusedTab[position] = sectionHead;
            }
            ...
            BitEditor bitmask = new BitEditor(song.FocusedTab.BeatCount * (tuningCount + 3));
            ...
            foreach (Step step in song.FocusedTab)
            {
                ...
                {
                    Beat beat = (Beat)step;
                    beat.BeatDivision = new MusicalTimespan(1, 8);
                    int noteLengthCode = bitmask.ReadBitsAsNumber(bitmaskI, 3);
                    bitmaskI += 3;
                    MusicalTimespan sustainTime = new MusicalTimespan(getNoteLength(noteLengthCode), 8);
                    for (int stringI = 0; stringI < tuningCount; stringI++)
                    {
                        if (bitmask[bitmaskI])
                        {
                            beat[new Fret(stringI, tabData[tabdataI])] = sustainTime;
                            tabdataI++;
                        }
                        bitmaskI++;
                    }
